...
export default class EndlessWorld {
    ...
    async loadEnvironmentSolids() {
        ...
        try {
            ...
            console.log("Creating platform and cubes...");
            ...
            platform.position.y = -0.2; // Position the platform just below 0
            platform.receiveShadows = true;
            platform.checkCollisions = true;
            platform.isPickable = true;
    
    // Create a line of cubes on the platform
    const cubeSize = 2; // Make cubes twice as large
    const numberOfCubes = 30; // Increase the number of cubes
    for (let i = 0; i < numberOfCubes; i++) {
        const cube = BABYLON.MeshBuilder.CreateBox(`cube${i}`, { size: cubeSize }, this.scene);
        cube.position.x = i * (cubeSize + 2); // Increase spacing between cubes
        cube.position.y = Math.random() * 5; // Random height position
        cube.receiveShadows = true;
        cube.checkCollisions = true;
        cube.isPickable = true;
      //  this.shadowGenerator.addShadowCaster(cube); // Add the cube to the shadow generator
    }
